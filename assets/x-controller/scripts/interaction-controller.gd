...
func check_interactions():
	...
	if result:
		var owner_node = result.collider.get_parent()
		#print(owner_node.name)
		if owner_node.has_method("interact"):

			if _look_at_modifier :

				if owner_node.interaction_object :
					#print(result.collider.interaction_object.get_path())
					_look_at_modifier.active = true
					#look_at_modifier.influence = 1.0
					_look_at_modifier.target_node = owner_node.interaction_object.get_path()
			...
			if owner_node.has_method("get_interaction_description"):
				_interact_label.text = owner_node.get_interaction_description()

			#if owner_node.has_method("is_open"):
				#if owner_node.is_open():
			...
			if _input_controller.interact :
				_animation_controller.animate_interactable(owner_node)
				owner_node.interact()
