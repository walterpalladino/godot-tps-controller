...
func update_weapons():
	...
	if Input.is_action_just_pressed("weapon1_enabled") :
		...
		if weapon1_enabled:
			animation_tree.set("parameters/Armed/transition_request", "weapon1_state")
		else:
...
func animate_locomotion():
	...
	if was_on_air:
		...
		if weapon1_enabled:
			animation_tree.set("parameters/Weapon1Locomotion/blend_position", abs(velocity.x) / max_character_speed_ground)
			animation_tree.set("parameters/Weapon1State/transition_request", "locomotion_state")
		else:
		...
		if weapon1_enabled:
			animation_tree.set("parameters/weapon1_landed/request", AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE)
		else:
		...
		if weapon1_enabled:
			animation_tree.set("parameters/Weapon1Crouch/blend_position", abs(velocity.x) / max_character_speed_crouched)
			animation_tree.set("parameters/Weapon1State/transition_request", "crouch_state")
		else:
...
func animate_on_air():
	...
	if weapon1_enabled:
		animation_tree.set("parameters/Weapon1Air/blend_position", abs(velocity.x) / max_character_speed_on_air)
		animation_tree.set("parameters/Weapon1State/transition_request", "on_air_state")
	else:
